...
class WebScraper:
    ...
    def process_items(self, items: List[Dict]) -> None:
        # 1. 筛选出所有Wiley的条目
        wiley_items = [item for item in items if item.get("publisher") == "Wiley"]
        print(f"find {len(wiley_items)} Wiley items.")
        # 2. 加载已处理的DOI列表
        processed_dois = set()
        extracted_data = []
        extracted_path = "./Data/wiley_extracted.json"
        # 检查并清理text为空的条目
        try:
            ...
            print("📋 wiley_extracted.json is not found, start from empty.")
            pass
        # 3. 筛选出未处理的条目
        unprocessed_items = []
        for item in wiley_items:
            if item.get("doi") not in processed_dois:
                unprocessed_items.append(item)
        total_processed = len(processed_dois)
        total_unprocessed = len(unprocessed_items)
        print(f"Statistics:")
        print(f"   - Total Wiley items: {len(wiley_items)}")
        print(f"   - Processed items: {total_processed}")
        print(f"   - Unprocessed items: {total_unprocessed}")
        if total_unprocessed == 0:
            print("✅ All Wiley items are processed.")
            return
        print(f"🚀 Start processing {total_unprocessed} unprocessed Wiley items...")
    ...
    def save_results(self, 
                    final_data_file: str = "./Data/wiley_extracted.json",
                    bad_data_file: str = "./Data/wiley_bad.json"):
        try:
