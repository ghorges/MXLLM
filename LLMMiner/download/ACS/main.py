...
class WebScraper:
    ...
    def process_items(self, items: List[Dict]) -> None:
        # 1. 筛选出所有ACS的条目
        acs_items = [item for item in items if item.get("publisher") == "ACS"]
        print(f"find {len(acs_items)} ACS items.")
        # 2. 加载已处理的DOI列表
        processed_dois = set()
        extracted_data = []
        extracted_path = "./Data/acs_extracted.json"
        # 检查并清理text为空的条目
        try:
            ...
            print("📋 acs_extracted.json is not found, start from empty.")
            pass
        # 3. 筛选出未处理的条目
        unprocessed_items = []
        for item in acs_items:
            if item.get("doi") not in processed_dois:
                unprocessed_items.append(item)
        total_processed = len(processed_dois)
        total_unprocessed = len(unprocessed_items)
        print(f"Statistics:")
        print(f"   - Total ACS items: {len(acs_items)}")
        print(f"   - Processed items: {total_processed}")
        print(f"   - Unprocessed items: {total_unprocessed}")
        if total_unprocessed == 0:
            print("✅ All ACS items are processed.")
            return
        print(f"🚀 Start processing {total_unprocessed} unprocessed ACS items...")
    ...
    def save_results(self, 
                    final_data_file: str = "./Data/acs_extracted.json",
                    bad_data_file: str = "./Data/acs_bad.json"):
        try:
